...
Constants can be built from other constants using a **constant expression** - any computation the compiler can evaluate ahead of time:
...
Unlike `let` bindings, a `const` has no fixed memory address - the compiler *inlines* its value wherever it is used, so there is zero runtime cost.
