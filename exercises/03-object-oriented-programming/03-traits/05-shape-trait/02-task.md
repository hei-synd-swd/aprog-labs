...
3. **Write a function** `fn compare_areas(a: &impl Shape, b: &impl Shape) -> String` that returns which shape has a larger area: `"First is larger"`, `"Second is larger"`, or `"Equal areas"`.

## Expected Result

`compare_areas(&rect, &circle)` compares areas of different shapes using a single function.
