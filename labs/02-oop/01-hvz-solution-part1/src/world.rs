...
impl World {
    ...
    fn tick_infection(&mut self) {
        ...
        for (i, human) in self.humans.iter().enumerate() {
            ...
            let nearby = self.zombies.iter().filter(|z| {
    ...
    fn tick_decay(&mut self) {
        for zombie in &mut self.zombies {
            let stamina = zombie.stamina();
            if stamina > ZOMBIE_DECAY {
                zombie.set_stamina(stamina - ZOMBIE_DECAY);
            } else {
                zombie.set_stamina(0);
            }
        }
        self.zombies.retain(|z| z.is_alive());
    }
