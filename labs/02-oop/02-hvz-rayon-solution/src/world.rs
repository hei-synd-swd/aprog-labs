...
impl World {
    ...
    pub fn count_humans(&self) -> usize {
        self.entities.par_iter().filter(|e| e.is_human()).count()
    }
    ...
    fn tick_infection(&mut self) -> Duration {
        ...
        let to_convert: Vec<(usize, Position)> = self
            ...
            .filter_map(|(i, entity)| {
                ...
                let nearby = self.entities.par_iter().filter(|e| {
                    if e.is_human() {
                        return false;
                    }
        ...
        for (_, pos) in to_convert {
            self.entities.push(Box::new(crate::zombie::Zombie::new(pos)));
        }
        start.elapsed()
    }
    ...
    fn tick_decay(&mut self) -> Duration {
        let start = Instant::now();
        self.entities.par_iter_mut().for_each(|e| e.tick());
